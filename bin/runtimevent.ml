    ...
    let com = compute (ev_end (Event.Timestamp.to_int64 ts) phase) in
      match com with
        |Ok (p, t) ->
          (match p with
            | EV_EXPLICIT_GC_SET ->
              H.observe P.explicit_gc_set t
            |	EV_EXPLICIT_GC_STAT ->
              H.observe P.explicit_gc_stat t
            |	EV_EXPLICIT_GC_MINOR ->
              H.observe P.explicit_gc_minor t
            |	EV_EXPLICIT_GC_MAJOR ->
              H.observe P.explicit_gc_major t
            |	EV_EXPLICIT_GC_FULL_MAJOR ->
              H.observe P.explicit_gc_full_major t
            |	EV_EXPLICIT_GC_COMPACT ->
              H.observe P.explicit_gc_compact t
            |	EV_MAJOR ->
              H.observe P.major t
            |	EV_MAJOR_SWEEP ->
              H.observe P.major_sweep t
            |	EV_MAJOR_MARK_ROOTS ->
              H.observe P.major_mark_roots t
            |	EV_MAJOR_MARK ->
              H.observe P.major_mark t
            |	EV_MINOR ->
              H.observe P.minor t
            |	EV_MINOR_LOCAL_ROOTS ->
              H.observe P.minor_local_roots t
            |	EV_MINOR_FINALIZED ->
              H.observe P.minor_finalized t
            |	EV_EXPLICIT_GC_MAJOR_SLICE ->
              H.observe P.explicit_gc_major_slice t
            |	EV_FINALISE_UPDATE_FIRST ->
              H.observe P.finalise_update_first t
            |	EV_FINALISE_UPDATE_LAST ->
              H.observe P.finalise_update_last t
            |	EV_INTERRUPT_REMOTE ->
              H.observe P.interrupt_remote t
            |	EV_MAJOR_EPHE_MARK ->
              H.observe P.major_ephe_mark t
            |	EV_MAJOR_EPHE_SWEEP ->
              H.observe P.major_ephe_sweep t
            |	EV_MAJOR_FINISH_MARKING ->
              H.observe P.major_finish_marking t
            |	EV_MAJOR_GC_CYCLE_DOMAINS ->
              H.observe P.major_gc_cycle_domains t
            |	EV_MAJOR_GC_PHASE_CHANGE ->
              H.observe P.major_gc_phase_change t
            |	EV_MAJOR_GC_STW ->
              H.observe P.major_gc_stw t
            |	EV_MAJOR_MARK_OPPORTUNISTIC ->
              H.observe P.major_mark_opportunistic t
            |	EV_MAJOR_SLICE ->
              H.observe P.major_slice t
            |	EV_MAJOR_FINISH_CYCLE ->
              H.observe P.major_finish_cycle t
            |	EV_MINOR_CLEAR ->
              H.observe P.minor_clear t
            |	EV_MINOR_FINALIZERS_OLDIFY ->
              H.observe P.minor_finalizers_oldify t
            |	EV_MINOR_GLOBAL_ROOTS ->
              H.observe P.minor_global_roots t
            |	EV_MINOR_LEAVE_BARRIER ->
              H.observe P.minor_leave_barrier t
            |	EV_STW_API_BARRIER ->
              H.observe P.stw_api_barrier t
            |	EV_STW_HANDLER ->
              H.observe P.stw_handler t
            |	EV_STW_LEADER ->
              H.observe P.stw_leader t
            |	EV_MAJOR_FINISH_SWEEPING ->
              H.observe P.major_finish_sweeping t
            |	EV_MINOR_FINALIZERS_ADMIN ->
              H.observe P.minor_finalizers_admin t
            |	EV_MINOR_REMEMBERED_SET ->
              H.observe P.minor_remembered_set t
            |	EV_MINOR_REMEMBERED_SET_PROMOTE ->
              H.observe P.minor_remembered_set_promote t
            |	EV_MINOR_LOCAL_ROOTS_PROMOTE ->
              H.observe P.minor_local_roots_promote t
            |	EV_DOMAIN_CONDITION_WAIT ->
              H.observe P.domain_condition_wait t
            |	EV_DOMAIN_RESIZE_HEAP_RESERVATION ->
              H.observe P.domain_resize_heap_reservation t)
          |Error _ -> ()
...
let main port argv =
  ...
  in
  Unix.sleepf 0.5;
  Lwt.async@@(fun() -> tracing (U.child_alive proc) (Some (".", proc)));
  let config = Prometheus_unix.config port in
  Lwt_main.run @@Lwt.choose (Prometheus_unix.serve config);
  Printf.printf "\n";
  0

let prom_port = Arg.(value & opt int 9090 & info ["p"; "port"] ~docv:"PORT")
